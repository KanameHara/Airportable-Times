...
const AirportPostShow: FC = () => {
  ...
  // const { placeID, postID } = router.query; // 動的ページだと404エラーになるので一旦コメントアウト
  ...
  const handlePreviousClick = () => {
    setCurrentImageIndex((prevIndex) => 
      prevIndex === 0 ? postDate.images.length - 1 : prevIndex - 1
    );
  };
  ...
  const handleNextClick = () => {
    setCurrentImageIndex((prevIndex) => 
      prevIndex === postDate.images.length - 1 ? 0 : prevIndex + 1
    );
  };
  ...
  return (
    <div>
      ...
      <Box p={5} mt={10} shadow="md" borderWidth="1px" borderRadius="md" width="50%" mx="auto">
        <Text mt={5} ml={10} fontWeight="bold">{postDate.title}</Text>
        <Box mt={5} ml={10} display="flex" alignItems="center">
          <Button onClick={handlePreviousClick}>&lt;</Button>
          <Image src={postDate.images[currentImageIndex]}
        ...
        <Flex>
          ...
          <Text mt={5} ml={5} fontWeight="bold">{postDate.taking_at}</Text>
        </Flex>
        <Flex>
        ...
        <Flex>
          ...
          <Text mt={5} ml={5} fontWeight="bold">{postDate.location}</Text>
        </Flex>
        <div style={{ marginTop: '15px', marginBottom: '50px' }}>
          <GoogleMap
            mapContainerStyle={containerStyle}
            center={{ lat: postDate.taking_position_latitude, lng: postDate.taking_position_longitude }}
            zoom={15}
          >
            <Marker position={{ lat: postDate.taking_position_latitude, lng: postDate.taking_position_longitude }}>
              <InfoWindow position={{ lat: postDate.taking_position_latitude, lng: postDate.taking_position_longitude }}>
                <div>撮影場所</div>
              </InfoWindow>
            </Marker>
          </GoogleMap>
        ...
        <Flex>
          <Text mt={2} ml={20} fontWeight="bold">{postDate.comment}</Text>
        </Flex>
